...
def plot_read_length_distribution2(read_length_distribution, mean_read_length):
    plt.bar(read_length_distribution.keys(), read_length_distribution.values(), width=1.0, color='skyblue', label='Reads')
    # Plotting red dashed line for the mean of normal reads
    plt.axvline(x=mean_read_length, color='black', linestyle='--', linewidth=1.5)
    # Displaying the mean value of normal reads as text
...
def amber_plot(bam_file_path):
    ...
    cmd = ["/home/benjguin/.conda/envs/My_conda/bin/pydamage  --outdir ",pydamage_dir," analyze ",bam_file_path," --group -p ", str(threads)]  # Group argument
...
for i in ["A"]:
            ...
            if PMD == "YES":
             ...
             plot_read_length_distribution2(read_length_distribution,mean_read_length)
            ...
            try:
             ...
             print("Amber error:")
             plt.text(0.5, 0.5, "Amber error", horizontalalignment='center', verticalalignment='center', fontsize=12, color='red')
             plt.axis('off')
             Amber="NO"
             print("Running pydamage...")
             pydamage_dir="pydamage_"+re.sub(".bam","",os.path.basename(bam_file_path))
            ...
            plt.subplot(3, 2, 5)
            try:
             PMDscore_plot=PMDscore_plot(tab_bam_file_PMDscores)
             # This is the plot
             PMDcore_plot[0]
             PMD_score=PMDcore_plot[1]
             PMD_perc_score_sup_3=PMDscore_plot[2]
             PMD_perc_score_sup_1pt5=PMDscore_plot[3]
            except:
             print("PMD_error:")
             plt.text(0.5, 0.5, "PMDtools error", horizontalalignment='center', verticalalignment='center', fontsize=12, color='red')
             plt.axis('off')
             PMD="NO"
             PMD_score=0
            ...
            if Amber=="YES":
             ...
             Sum_scores=read_length_distribution_score+evenness_score+PMD_score
